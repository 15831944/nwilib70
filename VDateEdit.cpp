...
CVDateTimeEdit::CVDateTimeEdit()
{
    m_strMask = _T("DD/MM/YYYY hh:mm:ss");
}

void CVDateTimeEdit::CVDateEdit(CWnd *Parent, unsigned int ID, LPCTSTR ErrMsg, CWnd *pCompareDate)
{
	Setup(Parent, ID, ErrMsg, pCompareDate);
}

void CVDateTimeEdit::Setup(CWnd *Parent, unsigned int ID, LPCTSTR ErrMsg, CWnd *pCompareDate)
{
	m_ErrMsg = ErrMsg;
...
BEGIN_MESSAGE_MAP(CVDateTimeEdit, CEdit)
    ON_WM_CONTEXTMENU()
    //{{AFX_MSG_MAP(CVDateTimeEdit)
    ON_WM_CHAR()
    ON_WM_KEYUP()
    ON_WM_KEYDOWN()
    ON_WM_MOUSEMOVE()
...
///////////////////////////////////////////////////////////////////////////
// CVDateTimeEdit message handlers

void CVDateTimeEdit::OnChar(UINT nChar, UINT /*nRepCnt*/, UINT /*nFlags*/) 
{
    CString old;
    GetWindowText(old);
    int pos;
...
void CVDateTimeEdit::OnKeyUp(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    CEdit::OnKeyUp(nChar, nRepCnt, nFlags);
}

void CVDateTimeEdit::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags) 
{
    switch (nChar) {
        ...
        case VK_UP: case VK_DOWN:
            {
                ...
                if (pos < m_strMask.GetLength()) {
                    ...
                    dt.SetDateTime(t.tm_year, t.tm_mon, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec);
...
void CVDateTimeEdit::OnMouseMove(UINT nFlags, CPoint point) 
{
...
void CVDateTimeEdit::PreSubclassWindow() 
{
    SetDateTime(COleDateTime::GetCurrentTime());
    CEdit::PreSubclassWindow();
}

void CVDateTimeEdit::SetDateTime(COleDateTime dt)
{
    CString str;
    ...
    if (pOwner != NULL) 
		pOwner->PostMessage(WM_DATETIME_EDIT, (WPARAM)DTE_DATETIMECHANGED, (LPARAM)0);
}

COleDateTime CVDateTimeEdit::GetDateTime()
{
    CString str;
    GetWindowText(str);
...
COleDateTime CVDateTimeEdit::GetDateTime(CString str)
{
    tm t;
    COleDateTime dt;
...
void CVDateTimeEdit::OnContextMenu(CWnd*, CPoint)
{
}

void CVDateTimeEdit::SetMask(CString mask) {
