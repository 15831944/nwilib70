...
BOOL CFifo::SetupSql(CString &Portfolios, CString &Condition)
{
	...
	m_OraLoader.GetSql().Format(
		"SELECT A.PORTFOLIO, B.ASS_COUNTRY, A.ASSET_CODE, A.DEAL_TYPE, A.TRADE_DATE, A.TRANS_NUM, "
		"A.TICKET_NUM, ACTUAL_CLOSING, A.TRANS_TYPE, NVL(X.NEW_CURRENCY, A.CURRENCY), A.TRANS_DIRECTION, "
		...
		"C.TRANS_NUM, C.PRICE, C.TRANS_DIRECTION, AMORT_FACT, B.ASS_TYPE, A.CURRENCY, ASS_CONV_TYPE, "
		"A.TRADE_DATE "
		"FROM SEMAM.NW_TR_TICKETS A, SEMAM.NW_ASSETS B, SEMAM.NW_TR_TICKETS C, SEMAM.NW_TR_TICKETS D, "
		"SEMAM.V_ASSET_RATES E, SEMAM.NW_PORTFOLIOS F, SEMAM.NW_TR_INV G, SEMAM.NW_COUNTRIES X "
	...
	m_OraLoader.GetSql() += 
		...
		"AMORT_FACT, B.ASS_TYPE, A.CURRENCY, ASS_CONV_TYPE, A.TRADE_DATE ";
	m_OraLoader.GetSql() += "FROM SEMAM.NW_TR_TICKETS A, SEMAM.NW_ASSETS B, SEMAM.NW_TR_TICKETS D, "
		"SEMAM.V_ASSET_RATES E, SEMAM.NW_PORTFOLIOS F, SEMAM.NW_TR_INV G, SEMAM.TEMP_EXERCISING_OPTIONS O ";
	...
	m_OraLoader.GetSql() += 
		...
		"G.AMORT_FACT*A.NOM_AMOUNT, A.PRICE/DECODE(D.ASS_TYPE, 'EQUITY TYPE CAL', 1, 100), "
		"NVL(B.FXRATE, NVL(M.FXRATE*NVL(L.CROSS_RATE, 1), C.EXRATE)), B.TRANS_NUM, "
		"NVL(B.PRICE, 0)/DECODE(D.ASS_TYPE,'EQUITY TYPE CAL',1,100), 'O', H.AMORT_FACT, "
	...
	m_OraLoader.GetSql() += 
		...
		"SEMAM.NW_TR_INV E, SEMAM.NW_PORTFOLIOS F, SEMAM.V_ASSET_RATES G, SEMAM.V_ASSET_RATES H, "
	...
	m_OraLoader.GetSql() += 
		"FROM SEMAM.V_ASSET_RATES A, SEMAM.NW_ASSETS B, SEMAM.NW_EXRATES C, SEMAM.NW_PORTFOLIOS F ";
