...
BOOL COXMultiComboBox::SetColumnMaxWidth(int nColIndex, int nChars)
{
	const int DEFAULT_CHAR_WIDTH = 8;
	int nWidth;
	
	if(nColIndex >= m_nColumnCount || nColIndex < -1)
		return FALSE;
	
	nWidth = nChars*DEFAULT_CHAR_WIDTH;
	if(nWidth > GetColumnWidth(nColIndex))
		SetColumnWidth(nColIndex, nWidth);
...
int COXMultiComboBox::GetCurSel()
{
	int nIndex;
	CString Text;

	nIndex = CComboBox::GetCurSel();
	GetLBText(nIndex, GetMasterColumn(), Text);
...
void COXMultiComboBox::GetSelString(CString& rString)
{
	int nIndex;
	...
	if (pRowData != NULL)
	{
		rString = pRowData->GetColumnString(GetMasterColumn());
		return;
	}
	else
		TRACE0("In COXMultiComboBox::GetLBText : GetRowData() returned NULL.\n");
...
int COXMultiComboBox::InsertString(int nIndex, LPCTSTR* lpszString, int nNumStrings)
{
	...
	if((nRetVal = CComboBox::InsertString(nIndex,lpszString[m_nMasterColumn])) != CB_ERR)
	{
		...
		for(int nColIndex=0; nColIndex < nNumStrings; nColIndex++)
		{
			...
			SetColumnMaxWidth(nColIndex, _tcslen(lpszString[nColIndex]));
		}
...
int COXMultiComboBox::InsertString(int nIndex, CStringArray &StringArray)
{
	int nRetVal;

	// return CB_ERR if invalid number of strings
	if(StringArray.GetCount() > m_nColumnCount || StringArray.GetCount() < 1)
		return CB_ERR;

	// Actually Inserts the item with master column text
	if((nRetVal = CComboBox::InsertString(nIndex, StringArray.GetAt(m_nMasterColumn))) != CB_ERR)
	{
		COXRowData* pRowData = new COXRowData(m_nColumnCount);
		// Initialize the column strings
		for(int nColIndex=0; nColIndex < StringArray.GetCount(); nColIndex++)
		{
			pRowData->SetColumnString(nColIndex, StringArray.GetAt(nColIndex));
			SetColumnMaxWidth(nColIndex, StringArray.GetAt(nColIndex).GetLength());
		}
		SetRowData(nRetVal,pRowData);
		AdjustToFitSize();
	}
...
int COXMultiComboBox::InsertString(int nIndex, LPCTSTR lpszString)
{
	...
	if((nRetVal = CComboBox::InsertString(nIndex,lpszString)) != CB_ERR)
	{
		...
		SetColumnMaxWidth(m_nMasterColumn, _tcslen(lpszString));
...
int COXMultiComboBox::AddString(CStringArray &StringArray) 
{
	int nRetVal;
	...
	// Adds the item with master column text
	if((nRetVal = CComboBox::AddString(StringArray.GetAt(m_nMasterColumn))) != CB_ERR)
	{
		COXRowData* pRowData = new COXRowData(m_nColumnCount);
		// Initialize the column strings
		for(int nColIndex=0; nColIndex < StringArray.GetCount(); nColIndex++)
...
int COXMultiComboBox::AddStringNone() 
{
	int nRetVal;

	// Adds the item with master column text
	if((nRetVal = CComboBox::AddString(NONE)) != CB_ERR)
	{
		COXRowData* pRowData = new COXRowData(m_nColumnCount);
		// Initialize the column strings
		for(int nColIndex=0; nColIndex < m_nColumnCount; nColIndex++)
		{
			pRowData->SetColumnString(nColIndex, NONE);
			SetColumnMaxWidth(nColIndex, _tcslen(NONE));
		}
		SetRowData(nRetVal,pRowData);
		AdjustToFitSize();
	}
...
void COXMultiComboBox::OnDropdown() 
{
    RecalcDropWidth();
}
...
void COXMultiComboBox::RecalcDropWidth()
{
