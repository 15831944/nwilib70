...
public:
	...
	void SetWithNone(BOOL bWithNone) { m_bWithNone = bWithNone; }
	...
	BOOL SetColumnMaxWidth(int nColIndex, int nChars); 
	// --- In  : nColWidth : Columns Width to set ,if -1, column width of each
	//						 column is set.
	//			 nChars : in number of charaters
	// --- Out : 
	// --- Returns : wether successfull or not 
	// --- Effect : Sets the width to the given column
	...
	void GetSelString(CString &rString);
	...
	int AddString(CStringArray &StringArray);
	...
	int InsertString(int nIndex, CStringArray &StringArray);
